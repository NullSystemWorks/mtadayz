...
function clickPanelButton (button, state)
	...
	if element then 
		...
		if info and info == "login" then
			...
			local gender = getElementData(getLocalPlayer(),"gender")
		...
		elseif info and info == "register" then  
				...
				local male_gender = guiRadioButtonGetSelected(male)
				local female_gender = guiRadioButtonGetSelected(female)
				if (guiRadioButtonGetSelected(male)) or (guiRadioButtonGetSelected(female)) then
					if (guiRadioButtonGetSelected(male)) then
						setElementData(getLocalPlayer(),"gender",true)
					elseif (guiRadioButtonGetSelected(female)) then
						setElementData(getLocalPlayer(),"gender",false)
					end
				if not (tostring(username) == "") then
						if not (tostring(pass1) == "") then
							if pass1 == pass2 then

								triggerServerEvent("onClientSendRegisterDataToServer", getLocalPlayer(), username, pass1, gender)				
							else
