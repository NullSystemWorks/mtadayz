...
local AnimalSkins = {12,13,14,15,16}
...
function RespawnAnimalPerTimer()
	spawnDayZAnimals()
end
setTimer(RespawnAnimalPerTimer,7200000,1,ped,pedCol,x,y,z)
...
function createDeadAnimal(who)
	...
	--local weaponEquipped = getPedWeapon(source)
	--killPed(ped)
	...
	killPed(ped)
	for i, item in ipairs(itemTableAnimals) do
			local value = math.random(0,8)
