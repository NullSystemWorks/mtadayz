...
beagleSpawns = gameplayVariables["beaglespawns"]

banditoSpawns = gameplayVariables["banditospawns"]
...
{"British Assault Pack",ID,1,0,3}, -- [XR]
{"Camouflage Assault Pack",ID,1,0,3},
{"Camp Backpack",ID,1,0,3},
{"Krysset Backpack",ID,1,0,3},
{"Ghillie Pack",ID,1,0,3},
{"Light Backpack",ID,1,0,3},
{"Military Backpack",ID,1,0,3},
{"OS Pack",ID,1,0,3},
{"RSS Assault Pack",ID,1,0,3},
{"Soldier Pack",ID,1,0,3},
...
{568,4,1,1},
--{548,0,2,1}, -- CARGOBOB! [XR2] OUT BECAUSE CARGOBOB IS HELICRASH SIDE!
{490,4,1,1},
}
...
function spawnDayZVehicles()
	...
	for i,tent in ipairs(tentSpawns) do
		...
		-- Beagle
		local item_id = math.random(table.size(beagleSpawns))
		local x,y,z = beagleSpawns[item_id][1],beagleSpawns[item_id][2],beagleSpawns[item_id][3]
		hunter = createVehicle(511,x,y,z)
		vehCol = createColSphere(x,y,z,4)
		attachElements ( vehCol, hunter, 0, 0, 0 )
		setElementData(vehCol,"parent",hunter)
		setElementData(hunter,"parent",vehCol)
		...
		setElementData(vehCol,"MAX_Slots",20)
		--Engine + Tires
		local tires,engine,parts = getVehicleAddonInfos (511)
		setElementData(vehCol,"Tire_inVehicle",math.random(0,tires))
		setElementData(vehCol,"Engine_inVehicle",math.random(0,engine))
		setElementData(vehCol,"Parts_inVehicle",math.random(0,parts))
		--vehicle_indentifikation
...
function respawnDayZVehicle(id,x,y,z,veh,col,max_slots)
	...
	if id == 511 then
		local item_id = math.random(table.size(beagleSpawns))
		x,y,z = beagleSpawns[item_id][1], beagleSpawns[item_id][2], beagleSpawns[item_id][3]
	end
		destroyElement(veh)
		destroyElement(col)
