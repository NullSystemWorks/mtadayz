...
function doBackup ()
		outputChatBox ("BACKUP OF ALL VEHICLES, PLEASE STAND IDLE!",getRootElement(),255,0,0,true)
		saveallvehicles()
...
function createVehicleOnServerStart()
	...
	for i = 1, (getAccountData(vehicleManager,"vehicleamount") or 0) do
		...
		if wastedVehicle then
			...
			if getAccountData(vehicle,"model") == 511 then
				local item_id = math.random(table.size(beagleSpawns))
				xx,yy,zz = beagleSpawns[item_id][1], beagleSpawns[item_id][2], beagleSpawns[item_id][3]
			end
			setElementPosition(veh,xxx,yyy,zzz+1)
			setElementPosition(veh,xx,yy,zz+1)
