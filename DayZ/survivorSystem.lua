...
function backPackBack (dataName,oldValue)
	if getElementType(source) == "player" and dataName =="MAX_Slots" then
		...
		if newValue == 12 then
			elementBackpack[source] = createObject(3026,x,y,z) -- ACU Pack
		elseif newValue == 16 then 
			elementBackpack[source] = createObject(1248,x,y,z) -- Alice Backpack
		elseif newValue == 26 then
			elementBackpack[source] = createObject(1575,x,y,z) -- Czech Backpack
		elseif newValue == 50 then
			elementBackpack[source] = createObject(1252,x,y,z) -- Coyote Backpack
		-- NEW BACKPACKS!
		elseif newValue == 20 then
			elementBackpack[source] = createObject(ID,x,y,z) -- British Pack
		elseif newValue == 15 then
			elementBackpack[source] = createObject(ID,x,y,z) -- Camouflage Pack
		elseif newValue == 21 then
			elementBackpack[source] = createObject(ID,x,y,z) -- Camp Pack
		elseif newValue == 13 then
			elementBackpack[source] = createObject(ID,x,y,z) -- Krysset Pack
		elseif newValue == 40 then
			elementBackpack[source] = createObject(ID,x,y,z) -- Ghillie Pack
		elseif newValue == 22 then
			elementBackpack[source] = createObject(ID,x,y,z) -- Light Backpack
		elseif newValue == 45 then
			elementBackpack[source] = createObject(ID,x,y,z) -- Military Backpack
		elseif newValue == 35 then
			elementBackpack[source] = createObject(ID,x,y,z) -- OS Pack
		elseif newValue == 23 then
			elementBackpack[source] = createObject(ID,x,y,z) -- RSS Pack
		elseif newValue == 14 then
			elementBackpack[source] = createObject(ID,x,y,z) -- Soldier Pack
		elseif newValue == 8 then -- No Backpack
			return
		end
		if newValue == 26 then
...
{"Camouflage Clothing (Female)", MODEL ID}, --[XR]
{"Civilian Clothing (Female)", MODEL ID},
{"Survivor Clothing (Female)", MODEL ID},
}
	...
	for i, player in ipairs(getElementsByType("player")) do
		if getElementData(player,"logedin") then
			...
			if getElementData(player,"bandit") then
				if getElementData(player,"gender") then
					...
					if current == MODEL ID or current == MODEL ID then
						setElementModel(player,MODEL ID) -- Bandit 1 (Female)
					elseif current == MODEL ID then
						setElementModel(player,MODEL ID) -- Bandit 2 (Female)
					end
				end
			elseif getElementData(player,"humanity") == 5000 then
				if current == 73 or current == MODEL ID or current == MODEL ID or current == 179 or current == 287 or current == MODEL ID then
					setElementModel(player,210) -- Hero Skin (Male + Female)
				end
			else
