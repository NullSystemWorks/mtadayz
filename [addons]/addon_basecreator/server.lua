...
function onStart()
	if not fileExists("db/bases.db") then
		...
		if h then
		fileClose(h)
		outputDebugString("[BASECREATOR] DB File not found, created it!")
		end
		...
		outputDebugString("[BASECREATOR] Creating basic DB structure!")
	else
		db = dbConnect( "sqlite", "db/bases.db" )
		...
		local result = dbPoll( qh, 10000 )
			...
			outputDebugString("[BASECREATOR] Base objects loaded. TOTAL: "..tostring(count))
	end
end
